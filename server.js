...
async function sendTelegramNotification(message) {
  ...
  if (!token || token === 'YOUR_BOT_TOKEN_HERE' || !chatIdStr) {
...
function saveDb() {
  ...
  fs.writeFileSync(path.join(__dirname, 'store.db'), buffer);
}
...
app.post('/api/orders', (req, res) => {
  ...
  runQuery(
    'INSERT INTO orders (customer_name, phone, governorate, address_detail, product_id, product_name, quantity, total_price, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
  ...
  const storePhone = settingsMap.store_phone || config.store_phone;

  const telegramMsg = [
    `🛒 <b>طلب جديد - ${storeName}</b>`,
    `━━━━━━━━━━━━━━━━━━━━`,
    ``,
    `👤 <b>العميل:</b> ${customer_name}`,
    `📱 <b>الهاتف:</b> ${phone}`,
    `📍 <b>المحافظة:</b> ${governorate}`,
    `🏠 <b>العنوان التفصيلي:</b> ${address_detail || 'غير محدد'}`,
    ``,
    `━━━━━━━━━━━━━━━━━━━━`,
    `📦 <b>المنتج:</b> ${product.name}`,
    `🔢 <b>الكمية:</b> ${qty}`,
    `💰 <b>السعر الإجمالي:</b> ${total.toLocaleString('ar-IQ')} د.ع`,
    ``,
...
app.post('/api/admin/products', authMiddleware, upload.single('image'), (req, res) => {
  ...
  runQuery(
    'INSERT INTO products (name, description, price, original_price, image, category, stock, featured) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
...
app.put('/api/admin/products/:id', authMiddleware, upload.single('image'), (req, res) => {
  ...
  const image = req.file ? `/images/${req.file.filename}` : (req.body.image !== undefined ? req.body.image : existing.image);
  runQuery(
    'UPDATE products SET name=?, description=?, price=?, original_price=?, image=?, category=?, stock=?, featured=? WHERE id=?',
    [
      name || existing.name,
...
app.get('/api/admin/stats', authMiddleware, (req, res) => {
  ...
  const todayOrders = getOne("SELECT COUNT(*) as count FROM orders WHERE date(created_at) = date('now')").count;
  res.json({ totalProducts, totalOrders, pendingOrders, totalRevenue, todayOrders });
});
...
app.put('/api/admin/settings', authMiddleware, (req, res) => {
  ...
  function saveSetting(key, value) {
    ...
    if (existing) {
      runQuery(`UPDATE settings SET value = ? WHERE key = '${key}'`, [value]);
    } else {
